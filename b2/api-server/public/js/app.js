...
function renderLogin() {
  const credCard = pendingCredentials ? `
    <div class="credential-card">
      <h3>Phase 2 Handoff Complete — Save Your admin_jwt</h3>
      <div class="credential-row"><label>workspace_id</label><code>${esc(String(pendingCredentials.workspace_id))}</code></div>
      <div class="credential-row" style="margin-top:8px"><label>workspace</label><code>${esc(pendingCredentials.name)}</code></div>
      <div class="credential-row" style="margin-top:8px"><label>admin_jwt</label></div>
      ...
      <button class="btn btn-sm" style="margin-top:8px" onclick="copyText('${esc(pendingCredentials.token)}')">Copy admin_jwt</button>
      <div style="margin-top:12px;font-size:.75rem;color:var(--text-muted)">Send every deploy request with <code>Authorization: Bearer &lt;admin_jwt&gt;</code>. The workspace_id is already in REVIEW state — no further /api/auth/login step is needed.</div>
  ...
  return `<div class="login-page">
    <div class="login-card">
      ...
      <div class="login-tabs">
        <div class="login-tab active" onclick="switchLoginTab('handoff',this)">Phase 1 Handoff</div>
        <div class="login-tab" onclick="switchLoginTab('rejoin',this)">Rejoin Workspace</div>
      </div>
      <div class="login-panel active" id="panel-handoff">
        <form id="handoffForm">
          <div style="background:var(--bg-subtle,#1a1a1f);border:1px solid var(--border,#333);border-radius:6px;padding:12px;margin-bottom:12px;font-size:.8rem;color:var(--text-muted)">
            <div style="color:var(--text,#ccc);margin-bottom:6px"><strong>Deployment Hub — Phase 2</strong></div>
            Self-service workspace registration is disabled on this stage.<br>
            Submit the <code>deployment_hmac_secret</code> recovered from Phase 1 <code>admin-console.json</code> to <code>POST /api/phase1-handoff</code>. The response contains a REVIEW-state <code>workspace_id</code> and an <code>admin_jwt</code>. Use the <code>admin_jwt</code> as <code>Authorization: Bearer</code> on every deploy call — <code>/api/auth/login</code> is not involved.
          </div>
          <div class="form-group">
            <label class="form-label">deployment_hmac_secret</label>
            <input class="form-input" id="hmacSecret" type="password" autocomplete="off" placeholder="recovered from Phase 1 admin-console.json" required>
            <div style="color:var(--text-muted);font-size:.75rem;margin-top:4px">Never stored — exchanged once for a REVIEW-state workspace_id + admin_jwt.</div>
          </div>
          <button type="submit" class="btn btn-primary" id="handoffBtn" style="width:100%;justify-content:center;padding:10px">Request Phase 2 Handoff</button>
        </form>
      </div>
      <div class="login-panel" id="panel-rejoin">
        <form id="rejoinForm">
          <div class="form-group">
            <label class="form-label">admin_jwt</label>
            <textarea class="form-textarea" id="rejoinToken" rows="3" placeholder="eyJhbGciOiJIUzI1NiIs..." required></textarea>
            <div style="color:var(--text-muted);font-size:.75rem;margin-top:4px">Paste the admin_jwt returned by /api/phase1-handoff to resume your Phase 2 session.</div>
          </div>
...
function render() {
  if (!currentToken && !currentWorkspace) {
    ...
    const handoffForm = $('#handoffForm');
    if (handoffForm) handoffForm.addEventListener('submit', async e => {
      e.preventDefault();
      const hmacSecret = $('#hmacSecret').value.trim();
      if (!hmacSecret) { toast('Enter the deployment_hmac_secret from Phase 1 admin-console.json', 'error'); return; }
      try {
        const data = await api('/api/phase1-handoff', {method:'POST', body: JSON.stringify({hmac_secret: hmacSecret})});
        if (!data || !data.admin_jwt || !data.workspace_id) {
          toast('Handoff response missing admin_jwt or workspace_id', 'error');
          return;
        }
        const payload = parseJwtPayload(data.admin_jwt) || {};
        pendingCredentials = {
          name: payload.workspace || `phase2-ws-${data.workspace_id}`,
          token: data.admin_jwt,
          workspace_id: data.workspace_id,
          workspace: { id: data.workspace_id, name: payload.workspace, pipeline_state: 'REVIEW' },
        };
        render();
      } catch (e) { toast((e && e.error) || 'Handoff failed — verify your deployment_hmac_secret', 'error'); }
    });
    const rejoinForm = $('#rejoinForm');
    if (rejoinForm) rejoinForm.addEventListener('submit', e => {
      e.preventDefault();
      const token = $('#rejoinToken').value.trim();
      if (!token) { toast('Paste the admin_jwt returned by /api/phase1-handoff', 'error'); return; }
      const payload = parseJwtPayload(token);
      if (!payload || !payload.workspace) { toast('Invalid token format — expected the admin_jwt from phase1-handoff', 'error'); return; }
      currentToken = token;
      currentWorkspace = { name: payload.workspace, pipeline_state: payload.role === 'pipeline_admin' ? 'REVIEW' : 'DRAFT' };
