...
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql://darkharbor:user_stack_fake_pw@postgres:5432/darkharbor",
)
REDIS_URL = os.environ.get("REDIS_URL", "redis://redis:6379")
FLAG = os.environ.get("FLAG", "FLAG{fake_flag}")
HMAC_SECRET = os.environ.get("HMAC_SECRET", "user_stack_fake_hmac_0000000000")
SIGNING_KEY_SEED = os.environ.get("SIGNING_KEY_SEED", "user_stack_fake_seed")
...
async def internal_policy_override(request: Request):
    ...
    if not _validate_pipeline_state(req.workspace_id, "REVIEW"):
        return JSONResponse(
            status_code=409,
            content={
                "error": "pipeline_state_conflict",
                "detail": "workspace must be in REVIEW state",
                "hint": {
                    "next_step": "POST /api/phase1-handoff",
                    "required_body": {"hmac_secret": "deployment_hmac_secret from Phase 1 admin-console.json"},
                    "note": "Only the phase1 handoff provisions a REVIEW-state workspace. Self-registered workspaces default to DRAFT and cannot be promoted externally.",
                },
            },
        )
